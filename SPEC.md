# SPEC — WhatsApp Chat Exporter (local)
...
| **Audience** | A single technical user exporting **their own** WhatsApp chats from an Android phone to a local ZIP. Push-to-GitHub, run locally, no deployment. |
...
The mental model: *the phone already keeps a complete, encrypted backup of every chat; this tool copies that backup to the computer, unlocks it with the user's own key, converts it to readable files, and packages it.*
...
## §1 — Domain facts / the actual values

These are technical facts the build must encode correctly; getting them wrong corrupts the core flow.
...
| Decryption key | The **64-hexadecimal-character** key (= 32 bytes) shown when the user enables End-to-End encrypted backup with the *key* (not password) option |
| Key file (`key`) and contacts DB (`wa.db`) | Live in `/data/data/com.whatsapp/` → **require root, NOT used by this tool** |
| Exporter invocation (crypt15 + hex key) | `wtsexporter -a -k <64hexkey> -b msgstore.db.crypt15` |
| Contact-name source | A user-supplied **vCard** (`contacts.vcf`), since `wa.db` needs root. Enriched via `--enrich-from-vcards` |
| Built-in export caps (why we avoid that route) | ~40,000 msgs without media / ~10,000 with — **not** a limit of this tool |

**LOCKED INTERPRETATION — backup freshness.** WhatsApp's local `msgstore.db.crypt15` is only as recent as the last local backup (auto ~daily). The tool must instruct the user to run a manual backup (WhatsApp → Settings → Chats → Chat backup → **Back Up**) before running, and must report the backup file's modified-timestamp so the user knows how fresh the export is.

**LOCKED INTERPRETATION — key format.** Enabling E2E encrypted backups switches the local backup to crypt15, decryptable with the same 64-hex key. The tool accepts the key as 64 hex chars (case-insensitive, spaces stripped). It does **not** attempt the rooted `key`-file path.
...
## §3 — Data model (artifacts + invariants)

This is a pipeline tool, not a CRUD app; the "entities" are files moving through stages.
...
## §4 — Features / the flow, step by step
...
1. **Preflight (`env_check`)**
   ...
   - `adb` resolvable on PATH, else exit with install guidance + the network-settings note.
   - Exactly one authorized device via `adb devices`. If zero → prompt user to connect + enable USB debugging + accept the on-phone "Allow USB debugging?" dialog. If >1 → require `--device SERIAL`.
...
### Default behaviours (chosen for you — override via flags)
...
## §5 — Core logic internals

Most heavy lifting is delegated; the glue logic to lock:

**Key normalization (`keyutil`)** — pure function, unit-testable. Strips *only* known noise (ASCII + Unicode whitespace, zero-width chars `U+200B`–`U+200D`/`U+FEFF`, and wrapping smart-quotes), then validates strictly. It does **not** strip arbitrary characters — that would mask a real typo:
```
NOISE = ASCII/Unicode whitespace + {U+200B,U+200C,U+200D,U+FEFF} + smart-quotes/'"`
normalize(raw):
    s = "".join(c for c in raw if c not in NOISE).lower()
    if len(s) != 64 or not all(c in "0123456789abcdef" for c in s):
        raise InvalidKey
    return s
```

**Pull (`pull`)** — uses `subprocess` to call `adb [-s SERIAL] pull <remote> <localtemp>`. Resolves remote paths by probing candidate locations (§1 default first, then known OEM variants) via read-only `adb shell ls`; if none exist, fails with guidance and accepts `--db-path` / `--media-path` overrides. Never runs `adb shell` commands that write. Transport-level failures (device offline, connection reset) are retried up to **3 times with linear backoff (2s, 4s)**; logical failures (file-not-found) are **not** retried. After a backup pull, the local file size is compared to the remote size (`adb shell stat`/`du`); a mismatch is treated as a failed (partial) pull and retried.

**Decrypt/export (`decrypt_export`)** — builds and runs the `wtsexporter` command. Captures exit code + stderr; maps the known "invalid key / inflate" error to a friendly message. **LOCKED:** decryption is considered successful only if the exporter exits 0 *and* the expected output files exist — lock this with a test using a known-good fixture. The command is **never logged with the key in it** — any logged form replaces the key with `***` (see §10 logging).
...
write_csv(rows)
```
**LOCKED INTERPRETATION — phone-number matching:** numbers are normalized to digits-only (strip `+`, spaces, dashes) before joining vCard to chat participants, to maximise match rate. Lock with a test (e.g. `+91 98xxx` in vCard matches `9198xxx@s.whatsapp.net`).

---

## §6 — Interface surface (CLI contract)
...
| `--package NAME` | string | `com.whatsapp` | WhatsApp package to target (e.g. `com.whatsapp.w4b` for Business) |
| `--db-path PATH` | path | auto-probe | Override the on-device backup path (OEM-specific layouts) |
| `--media-path PATH` | path | auto-probe | Override the on-device media path |
| `--verbose` | flag | off | DEBUG logging (adb command lines, redacted) |
| `--keep-temp` | flag | off | Keep temp workdir for debugging |

**Exit codes:** `0` success · `1` preflight failure (python/adb/device/transport) · `2` invalid key · `3` no backup file found · `4` decryption failed · `5` packaging/IO error · `130` user-interrupted (Ctrl-C). (Each maps to a printed, actionable message; all are raised as typed exceptions — see §10.)

---

## §7 — Critical behaviours (don't get these wrong)
...
- **Secrets never enter Git.** `.gitignore` excludes keys, `*.crypt15`, `msgstore.db`, `wa.db`, `*.vcf`, `/output/`, `/.tmp/`, `*.zip` from the first commit. (See SECURITY.md.)
- **Disk safety with media.** Media can be large; check free space before pull and warn.
- **Guaranteed teardown.** Temp cleanup and key wipe run via `try/finally` (or a context manager) on every exit path — success, handled failure, and Ctrl-C (`KeyboardInterrupt` → exit `130`).
- **Log redaction backstop.** A logging filter redacts any 64-hex substring from every log record, so even an accidental key-in-message can't leak (see §10).
...
- `SecretRedactionFilter` replaces a 64-hex string in a log message with `***` (unit test).
- Ctrl-C mid-run: temp workdir is removed, no key/`*.crypt15` remains, exit `130` (simulate `KeyboardInterrupt`).
...
**Edge cases — see the §11 register for the full handled/deferred list.** Each "handled" row gets a test as it's built; "deferred" rows get a clear-failure test (correct exit code + actionable message), not silent degradation.
...
**Out of scope / resist these in v1:**
...
- WhatsApp Business (`com.whatsapp.w4b`) is reachable via `--package` but not separately tested in v1; "parallel space" / cloned-app installs are out of scope.
- Editing, redacting, or searching chats inside the tool (the HTML output already browses; leave analysis to the user).

---

## §10 — Design-pattern decisions (closed)

These were the open pattern questions; each is now locked.

**Package-level Facade — DECIDED: include.** A single `run_export(config) -> Path` function runs all six stages and returns the ZIP path. `export.py` is a thin shell that only parses flags, builds the config, calls `run_export`, and maps exceptions to exit codes. Benefits: the tool is importable/programmatic, end-to-end tests get a clean seam, and the CLI stays dumb. The orchestrator inside `run_export` is a pipeline coordinator, not a second facade — no facades-in-front-of-facades.

**Immutable context object — DECIDED.** A frozen dataclass `RunContext` holds non-secret config only (flags, resolved paths, device serial). Stages receive it read-only and *return* their outputs as values (e.g. `pull` returns the local backup path) rather than mutating shared state — preserving explicit pipes-and-filters data flow. **The key is never stored in `RunContext`** (a frozen dataclass's default `repr` would print it); it is passed as a separate argument only to `keyutil` (produces it) and `decrypt_export` (consumes it). Key-hygiene is thus structural, not just a convention.

**Typed exception hierarchy — DECIDED.** `WaeError(message, exit_code)` base, with `EnvError(1)`, `DeviceError(1)`, `InvalidKey(2)`, `NoBackupError(3)`, `DecryptionError(4)`, `PackagingError(5)`. Stages raise these; the `export.py` shell catches `WaeError`, prints `.message`, exits `.exit_code`. `KeyboardInterrupt` → exit `130`. This is the guard-clause pattern with exit codes bound to types, so §6's codes can't drift.

**adb retry policy — DECIDED.** Bounded retry in the `pull` adapter: 3 attempts, linear backoff (2s, 4s), **only** on transport-level errors (device offline/unauthorized-mid-run/connection reset). Logical errors (file-not-found) never retry. No exponential-jitter machinery — unwarranted for a local tool.

**Logging — DECIDED.** One configured `logging` logger; INFO→stdout by default, DEBUG (incl. redacted adb command lines) under `--verbose`. The key is never passed to the logger. Defense-in-depth: a `SecretRedactionFilter` scans every record and replaces any 64-hex substring with `***`. No telemetry — consistent with the no-network pillar.

**Teardown — DECIDED.** `run_export` wraps the pipeline in `try/finally` (or a context manager) so temp cleanup and key wipe always fire — success, handled failure, or Ctrl-C.

---

## §11 — Edge-case register (handled / deferred)

"Handled" = the tool does the right thing and a test locks it. "Deferred" = explicitly out of v1 scope, but must **fail clearly** (correct exit code + actionable message), never degrade silently.

| # | Edge case | Status | Behaviour |
|---|---|---|---|
| 1 | Zero chats in backup | Handled | Produces a valid empty export; `contacts.csv` has header only |
| 2 | Chat with only media | Handled | Rendered by the exporter; media included |
| 3 | Group member missing from vCard | Handled | Row emitted as `source=number-only` |
| 4 | Multiple attached devices | Handled | Requires `--device`; else exit `1` listing serials |
| 5 | Unauthorized device | Handled | Exit `1`; instructs user to accept the on-phone dialog |
| 6 | Media larger than free disk | Handled | Pre-pull size check; warn + offer abort |
| 7 | Non-ASCII contact names | Handled | UTF-8 throughout (CSV + HTML) |
| 8 | Device disconnect mid-pull (partial backup) | Handled | Size mismatch vs remote → treated as failed pull → retried (§5) |
| 9 | OEM non-default storage path | Handled | Probe candidate paths; else `--db-path`/`--media-path` overrides |
| 10 | Backup mid-write (just hit Back Up) | Handled | mtime within ~30s → warn it may still be writing, offer wait/abort |
| 11 | adb present but old/wrong version | Handled | `adb version` parsed; warn below known-good minimum (don't hard-block) |
| 12 | Key with zero-width / smart-quote noise | Handled | Hardened `normalize()` strips known noise only, then strict 64-hex (§5) |
| 13 | Unreadable/broken-symlink media files | Handled | Pull continues; warning summary of skipped files |
| 14 | Phone locked during pull (FBE) | Handled | Pull permission error → message to unlock the phone and retry |
| 15 | Output dir unwritable / no space for ZIP | Handled | Pre-package writability + space check → `PackagingError` (exit `5`) |
| 16 | Same-day ZIP already exists | Handled | Suffix `-2`, `-3`, … never overwrite (F6.2) |
| 17 | Ctrl-C mid-run | Handled | `try/finally` cleanup + key wipe; exit `130` (§10) |
| 18 | WhatsApp Business alongside regular app | Handled (flag) | Targets `com.whatsapp` by default; `--package com.whatsapp.w4b` for Business |
| 19 | Legacy crypt12/crypt14 backup | Deferred | Detect format → fail clearly, point to re-creating an E2E backup |
| 20 | "Parallel space" / cloned-app installs | Deferred | Fail clearly; not supported in v1 |
| 21 | Single chat huge enough to strain exporter memory | Deferred | Known limitation; revisit only if hit in practice |
| 22 | Timestamp/timezone rendering | Deferred to library | Exporter's default TZ handling; documented, not customized in v1 |
